...
Let's see the configuration in action on the running example from [Multi-Module Projects](/guide/concepts/multi-module): the `my-monorepo` workspace with the `common`, `auth`, `api`, `web` and `mobile` modules.
...
The other modules (`auth`, `api`, `web` and `mobile`) get the same treatment in their own `CHANGELOG.md` files as their versions cascade - see [Dependency Cascade](/guide/concepts/dependency-cascade).

## Next Steps

- [Release Notes](/guide/config/release) - The release notes counterpart
...
Ready to customize your release notes? Check out the [Release Notes](/guide/config/release) guide!
